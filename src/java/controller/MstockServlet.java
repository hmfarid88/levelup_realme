...
//                    String vendorpro="select sum(PURCHASE_PRICE) from vendor_stock where VENDOR=?";
//                    ps = con.prepareStatement(vendorpro);
//                    ps.setString(1, vendor);
//                    rs1=ps.executeQuery();
//                    rs1.next();
//                    Long totalvalue=rs1.getLong(1);
//                    String vendorpay="select sum(AMOUNT) from company_payment where COMPANY_NAME=?";
//                    ps = con.prepareStatement(vendorpay);
//                    ps.setString(1, vendor);
//                    rs2=ps.executeQuery();
//                    rs2.next();
//                    Long totalpay=rs2.getLong(1);
//                    Long vendorbalance=totalpay-totalvalue;
//                    if(vendorbalance<pprice){
//                        out.println("<center><br><h2>Sorry, Insufficient Balance !</h2></center>"); 
//                    }else{
                    String query1 = "insert into stock (MODEL, COLOR, IMEI, PURCHASE_PRICE, SELL_PRICE, VENDOR, AREA, STORE, BOSS, DATE)"
                            + "values (?,?,?,?,?,?,?,?,?, CURDATE())";
                    ...
                    if (b > 0) {
                        String query2="insert into vendor_stock (PRODUCT, PRODUCT_ID, PURCHASE_PRICE, SALE_PRICE, VENDOR, DATE)"
                                + "values (?,?,?,?,?, CURDATE())";
